...
void main() {
  group('VaultBloc - Fetch data from Firestore', () {
    ...
    blocTest<VaultBloc, VaultState>(
      ...
      build: () {
        when(mockVaultRemoteDataSource.getAllVaults()).thenAnswer((_) async {
          // LOG: Afficher les données brutes retournées par le mock de Firestore
          debugPrint('MOCK FIRESTORE RETURNING DATA: ');
          for (var model in vaultModels) {
            debugPrint(
                '  Vault ID: ${model.id}, Name: ${model.name}, CreatedAt: ${model.createdAt}');
          }
          return vaultModels;
        });
        return vaultBloc;
      ...
      expect: () => [
        ...
        isA<VaultLoaded>().having((state) {
          final loadedVaults = (state).vaults;
          // LOG: Afficher les entités dans l'état chargé du bloc
          debugPrint('BLOC LOADED STATE CONTAINS: ');
          for (var vault in loadedVaults) {
            debugPrint(
                '  Vault ID: ${vault.id}, Name: ${vault.name}, CreatedAt: ${vault.createdAt}');
          }
    ...
    test('VaultRepository transforms remote data correctly into entities',
      ...
      result.fold(
        ...
        (fetchedVaults) {
          ...
          debugPrint('TEST - Repository fetched and transformed data: ');
          ...
          for (int i = 0; i < fetchedVaults.length; i++) {
            ...
            expect(fetchedVaults[i].createdAt, vaultModels[i].createdAt,
                reason: 'CreatedAt does not match');
          }
        },
      );
    });
